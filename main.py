        ...
        if self.table[index] is not None:
            for item in self.table[index]:
                if item[0] == key:
                    ...
                    return item[1], time_taken_ms  # Return the value and time taken
        end_time = time.time()  # Stop timing
        time_taken_ms = (end_time - start_time) * 1000  # Convert to milliseconds
        return None, time_taken_ms  # Key not found, return None and time taken
...
class Graph:
    ...
    def add_song(self, song_name, song_metrics):
        if song_name not in self.adj_list:
            ...
            print(f"Song '{song_name}' already exists in the graph.")
